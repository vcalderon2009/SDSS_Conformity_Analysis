...
def data_shuffles_extraction(param_dict, proj_dict, pickle_ext='.p'):
    ...
    for gm in param_dict['mgroup_keys']:
        ...
        for prop in param_dict['prop_keys']:
            ...
            (   sigma_conf_dict,
                conf_mean      ,
                conf_std       ) = cstats.sigma_calcs(mcf_conf_sh,
                                        type_sigma=param_dict['type_sigma'],
                                        return_mean_std=True)
            # 'Conf + Seg'
            (   sigma_conf_seg_dict,
                conf_seg_mean      ,
                conf_seg_std       ) = cstats.sigma_calcs(mcf_conf_seg_sh,
                                        type_sigma=param_dict['type_sigma'],
                                        return_mean_std=True)
            ##
...
def mocks_data_extraction(param_dict, proj_dict, pickle_ext='.p'):
    ...
    for gm in param_dict['mgroup_keys']:
        ...
        for prop in param_dict['prop_keys']:
            ...
            (   sigma_conf_dict,
                conf_mean      ,
                conf_std       ) = cstats.sigma_calcs(mcf_mocks,
                                        type_sigma=param_dict['type_sigma'],
                                        return_mean_std=True)
            # 'Conf + Seg'
            (   sigma_conf_seg_dict,
                conf_seg_mean      ,
                conf_seg_std       ) = cstats.sigma_calcs(mcf_seg_mocks,
                                        type_sigma=param_dict['type_sigma'],
                                        return_mean_std=True)
            ##
